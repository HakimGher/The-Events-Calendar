    ...
    class Cal {
        ...
        public function save ($start, $end, $txt, $color, $id=null) {
            ...
            if ($id==null) {
              ...
              $sql = "INSERT INTO `user_events` (`user_id`,`evt_start`, `evt_end`, `evt_text`, `evt_color`) VALUES (?,?,?,?,?)";
              $data = [$user_id,$start, $end, $txt, $color];
            } else {
              $sql = "UPDATE `user_events` SET `evt_start`=?, `evt_end`=?, `evt_text`=?, `evt_color`=? WHERE `event_id`=?";
